...
export async function GET(request: NextRequest) {
	try {
		...
		if (!refresh) {
			...
			if (cached) {
				const data: HomepageStatsResponse =
					typeof cached === 'string' ? (JSON.parse(cached) as HomepageStatsResponse) : (cached as HomepageStatsResponse);
		...
		return withHeaders(
			NextResponse.json({ error: 'Failed to fetch homepage statistics' }, { status: 500 })
		);
	}
}

function withHeaders(response: NextResponse) {
	response.headers.set('Cache-Control', `s-maxage=3600, max-age=${CACHE_TTL_SECONDS}`);
	return response;
}

async function calculateHomepageStats(): Promise<HomepageStatsResponse> {
	const [platformStats, featuredOrganizations, popularSkills, recentActivity] = await Promise.all([
	...
	return { platformStats, featuredOrganizations, popularSkills, recentActivity };
}

async function calculatePlatformStats(): Promise<HomepageStatsResponse['platformStats']> {
	const [activeBounties, activeGrants] = await Promise.all([
		database.bounty.count({ where: { visibility: 'PUBLISHED', status: { in: ['OPEN'] } } }),
		database.grant.count({ where: { visibility: 'PUBLISHED', status: 'OPEN' } }),
	]);
	...
	const [bountyAgg, grantsForSum] = await Promise.all([
		database.bounty.aggregate({ _sum: { amount: true }, where: { winnersAnnouncedAt: { not: null } } }),
...
async function getFeaturedOrganizations(): Promise<HomepageStatsResponse['featuredOrganizations']> {
	...
	const [bountyAggByOrg, grantAggByOrg, orgs] = await Promise.all([
		...
		database.grant.findMany({
			where: { visibility: 'PUBLISHED', status: 'OPEN' },
			select: { organizationId: true, maxAmount: true, minAmount: true, id: true },
		}),
		database.organization.findMany({ select: { id: true, name: true, slug: true, logo: true } }),
	]);
	...
	return merged
		...
		.sort((a, b) => (b.totalValue === a.totalValue ? b.totalOpportunities - a.totalOpportunities : b.totalValue - a.totalValue))
		.slice(0, 3);
}

async function getPopularSkills(): Promise<HomepageStatsResponse['popularSkills']> {
...
async function getRecentActivity(): Promise<HomepageStatsResponse['recentActivity']> {
	const [recentSubmissions, recentApplications] = await Promise.all([
		database.submission.findMany({
			where: { status: { not: 'DRAFT' } },
			include: {
				submitter: { select: { firstName: true, lastName: true, username: true, avatarUrl: true } },
				bounty: { select: { id: true, title: true, organization: { select: { name: true } } } },
			},
		...
		database.grantApplication.findMany({
			where: { status: { not: 'DRAFT' } },
			include: {
				applicant: { select: { firstName: true, lastName: true, username: true, avatarUrl: true } },
				grant: { select: { id: true, title: true, organization: { select: { name: true } } } },
			},
			orderBy: { createdAt: 'desc' },
			take: 10,
		}),
	...
	const submissionActivities = recentSubmissions.map(s => ({
		...
		user: {
			...
			avatarUrl: s.submitter?.avatarUrl ?? null,
		},
		target: {
			id: s.bounty?.id ?? '',
