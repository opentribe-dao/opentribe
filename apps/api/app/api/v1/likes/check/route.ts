...
export async function GET(request: NextRequest) {
  try {
    ...
    const applicationIds = searchParams.get('applicationIds')?.split(',').filter(Boolean) || [];
    const submissionIds = searchParams.get('submissionIds')?.split(',').filter(Boolean) || [];
    ...
    const likedApplicationIds = new Set(likes.filter(l => l.applicationId).map(l => l.applicationId));
    const likedSubmissionIds = new Set(likes.filter(l => l.submissionId).map(l => l.submissionId));
