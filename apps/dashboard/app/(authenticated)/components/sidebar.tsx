...
const useNavData = () => {
  ...
  return {
    ...
    bottomActions: [
      {
        title: 'Settings',
        url: activeOrg ? `/org/${activeOrg.id}/settings` : '/settings',
        icon: SettingsIcon,
      },
      {
        title: 'Help',
            ...
            <SidebarGroupContent className="mt-3">
              {activeOrg ? (
                <div className="flex items-center justify-between py-2 px-3">
                  <span className="text-sm text-white/60">{activeOrg.name}</span>
                  <span className="text-xs text-white/40">0</span>
                </div>
              ) : (
                <div className="py-2 px-3">
                  <span className="text-xs text-white/40">No organization selected</span>
                </div>
              )}
            </SidebarGroupContent>
          </SidebarGroup>
