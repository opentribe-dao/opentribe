...
export default function OrganizationOnboardingPage() {
  ...
  const validateStep2 = () => {
    if (!formData.organizationName || !formData.organizationType || !formData.organizationDescription) {
      toast.error('Please fill in all required fields');
      return false;
    }
  ...
  const handleSubmit = async () => {
    try {
      ...
      const dashboardUrl = env.NEXT_PUBLIC_DASHBOARD_URL || 'http://localhost:3001';
    ...
    <div className="min-h-screen flex items-center justify-center p-4">
      <div className="max-w-2xl w-full">
        ...
        <div className="bg-zinc-900/95 backdrop-blur-md border border-white/10 rounded-2xl p-8">
          ...
          {/* Title */}
          <div className="mb-8">
            <h1 className="text-2xl font-semibold text-white mb-2">Tell Us About Your Organization</h1>
            <p className="text-white/60 text-sm">
              This information will be displayed on your public profile and help us connect you with the right talent.
            </p>
          </div>
          ...
          <div className="flex items-center gap-3 mb-8">
            <div className="flex items-center gap-2">
              <div className={`w-2 h-2 rounded-full ${currentStep >= 1 || session?.user?.profileCompleted ? 'bg-[#E6007A]' : 'bg-white/20'}`} />
              {!session?.user?.profileCompleted && <span className="text-xs text-white/60">Personal Details</span>}
            </div>
            <div className="flex-1 h-px bg-white/20" />
            <div className="flex items-center gap-2">
              <div className={`w-2 h-2 rounded-full ${currentStep >= 2 ? 'bg-white' : 'bg-white/20'}`} />
              <span className="text-xs text-white/60">Organization Details</span>
            </div>
            <div className="flex-1 h-px bg-white/20" />
            <div className="flex items-center gap-2">
              <div className={`w-2 h-2 rounded-full ${currentStep >= 3 ? 'bg-white' : 'bg-white/20'}`} />
              ...
              {session?.user?.profileCompleted && (
                <div className="bg-white/5 border border-white/10 rounded-lg p-4 mb-6">
                  <p className="text-sm text-white/80">
                    We've pre-filled your personal details from your existing profile. You can update them if needed.
                  </p>
                </div>
              )}
              {/* Name Fields */}
          ...
          {currentStep === 2 && (
            <div className="space-y-6">
              {/* Organization Name */}
              <div>
                <Label htmlFor="organizationName" className="text-white/80 mb-2">
                  Organization Name *
                </Label>
                <Input
                  id="organizationName"
              ...
              <div>
                <Label htmlFor="organizationDescription" className="text-white/80 mb-2">
                  Organization Description *
                </Label>
                <Textarea
              ...
              <div>
                <Label htmlFor="organizationWebsite" className="text-white/80 mb-2">
                  Organization Website *
                </Label>
                <Input
              ...
              <div>
                <Label htmlFor="organizationLocation" className="text-white/80 mb-2">
                  Organization Location *
                </Label>
                <Input
              ...
              <div>
                <Label className="text-white/80 mb-4 block">
                  Organization Logo <span className="text-xs text-white/40">(Optional)</span>
                </Label>
                <ImageUpload
              ...
              <div>
                <div className="flex items-center justify-between mb-4">
                  <Label className="text-white/80">
                    Team Members <span className="text-xs text-white/40">(Optional)</span>
                  </Label>
                  <Button
                    type="button"
                    variant="outline"
                ...
                {formData.teamMembers.length === 0 ? (
                  <div className="text-center py-8 bg-white/5 rounded-lg border border-white/10">
                    <p className="text-white/60 text-sm mb-3">No team members added yet</p>
                    <p className="text-white/40 text-xs">You can invite team members later from your dashboard</p>
                  </div>
                ) : (
                      ...
                      <div key={member.id} className="bg-white/5 rounded-lg p-4 border border-white/10">
                        <div className="flex items-start gap-4">
                          <div className="flex-1 space-y-4">
                            ...
                            <Select
                              value={member.role}
                              onValueChange={(value) => updateTeamMember(member.id, 'role', value as 'admin' | 'member')}
                            >
                              <SelectTrigger className="bg-white/5 border-white/20 text-white">
                                <SelectValue />
                              </SelectTrigger>
          ...
          <div className="flex justify-between items-center mt-8">
            <Button
              variant="ghost"
              onClick={() => router.push(env.NEXT_PUBLIC_DASHBOARD_URL || 'http://localhost:3001')}
              className="text-white/60 hover:text-white"
            >
              Save as Draft
            </Button>
