...
export default function ClaimProfilePage() {
  ...
  const submitVerification = async (
    ...
    try {
      ...
      if (data.status === "VERIFIED") {
        setClaimState({ step: "success", message: data.message });
        toast.success("Profile claimed successfully!");
      } else if (data.status === "PENDING") {
        setClaimState({ step: "pending", message: data.message });
  ...
  return (
    <div className="min-h-screen">
      <div className="container relative z-10 mx-auto max-w-3xl px-4 py-12">
            ...
            {profile.skills && profile.skills.length > 0 && (
              <div className="mt-4 flex flex-wrap gap-2">
                ...
                {profile.skills.length > 6 && (
                  <Badge className="border-0 bg-white/10 text-white/50" variant="secondary">
        ...
        <h1 className="mb-2 font-bold text-2xl text-white">Claim this profile</h1>
        <p className="mb-8 text-white/60">
          Verify your identity to link this ecosystem profile to your Opentribe account.
        </p>
        ...
        {!session?.user && (
          <Card className="mb-8 border-yellow-500/30 bg-yellow-500/10 backdrop-blur-md">
            <CardContent className="p-6">
              <p className="mb-4 text-white/80">You need to be signed in to claim a profile.</p>
              <AuthModal redirectTo={`/profile/claim/${slug}`}>
                <Button className="bg-[#E6007A] text-white hover:bg-[#FF1493]">Sign In to Continue</Button>
              </AuthModal>
            </CardContent>
          </Card>
        )}
        ...
        {claimState.step === "success" && (
          <Card className="border-green-500/30 bg-green-500/10 backdrop-blur-md">
            <CardContent className="p-8 text-center">
              ...
              <h2 className="mb-2 font-semibold text-xl text-white">Profile Claimed!</h2>
              <p className="mb-6 text-white/70">{claimState.message}</p>
              <div className="flex justify-center gap-4">
                <Link href={`/profile/${slug}`}>
                  <Button className="bg-[#E6007A] text-white hover:bg-[#FF1493]">View Your Profile</Button>
                </Link>
                <Link href="/settings">
        ...
        {claimState.step === "pending" && (
          <Card className="border-yellow-500/30 bg-yellow-500/10 backdrop-blur-md">
            <CardContent className="p-8 text-center">
              ...
              <h2 className="mb-2 font-semibold text-xl text-white">Claim Pending Review</h2>
              <p className="mb-6 text-white/70">{claimState.message}</p>
              <Link href={`/profile/${slug}`}>
            ...
            <CardContent className="p-6">
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="mt-1 text-sm text-white/70">{claimState.errorMessage}</p>
                  <Button className="mt-4" onClick={() => setClaimState({ step: "choose" })} size="sm" variant="outline">
                    Try Again
                  </Button>
                </div>
              </div>
        ...
        {/* Email verification */}
        {claimState.step === "verifying" && claimState.method === "EMAIL_VERIFICATION" && (
          <Card className="mb-8 border-white/10 bg-white/5 backdrop-blur-md">
            <CardContent className="p-6">
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1">
                  ...
                  <p className="mt-1 text-sm text-white/70">
                    ...
                    <span className="font-mono text-white/90">{claimState.maskedEmail}</span>
                  </p>
                  ...
                  <Button className="mt-3" onClick={() => setClaimState({ step: "choose" })} size="sm" variant="ghost">
        ...
        {/* Wallet signing */}
        {claimState.step === "verifying" && claimState.method === "WALLET_SIGNATURE" && (
          <Card className="mb-8 border-white/10 bg-white/5 backdrop-blur-md">
            <CardContent className="p-6 text-center">
              <Loader2 className="mx-auto mb-4 h-8 w-8 animate-spin text-[#E6007A]" />
              <p className="font-medium text-white">Waiting for wallet signature...</p>
              <p className="mt-2 text-sm text-white/60">Please approve the signing request in your wallet extension.</p>
              <Button className="mt-4" onClick={() => setClaimState({ step: "choose" })} size="sm" variant="ghost">
                Cancel
              </Button>
            </CardContent>
          </Card>
        ...
        {claimState.step === "choose" && session?.user && (
          <div className="space-y-4">
            {profile.github && (
              <Card className="border-white/10 bg-white/5 backdrop-blur-md transition-colors hover:border-white/20">
                <CardContent className="p-6">
                  <div className="flex items-start gap-4">
                    ...
                    <div className="flex-1">
                      <div className="flex items-center gap-2">
                        <h3 className="font-semibold text-white">Verify with GitHub</h3>
                        <Badge className="border-0 bg-green-500/20 text-green-400">Recommended</Badge>
                      </div>
                      <p className="mt-1 text-sm text-white/60">
                        We will verify that your linked GitHub account matches the profile&apos;s GitHub:{" "}
                        <span className="font-mono text-white/80">{profile.github}</span>
                      </p>
                      <Button
              ...
              <Card className="border-white/10 bg-white/5 backdrop-blur-md transition-colors hover:border-white/20">
                <CardContent className="p-6">
                  <div className="flex items-start gap-4">
                    ...
                    <div className="flex-1">
                      <h3 className="font-semibold text-white">Sign with Polkadot Wallet</h3>
                      <p className="mt-1 text-sm text-white/60">
                        Sign a message with a wallet address associated with this profile to prove ownership.
                      </p>
                      <p className="mt-2 text-white/40 text-xs">Requires Polkadot.js, Talisman, or SubWallet extension</p>
                      <Button className="mt-4" disabled={submitting} onClick={() => initiateClaim("WALLET_SIGNATURE")} variant="outline">
                        {submitting && claimState.method === "WALLET_SIGNATURE" ? (
                          <Loader2 className="mr-2 h-4 w-4 animate-spin" />
                        ) : (
              ...
              <Card className="border-white/10 bg-white/5 backdrop-blur-md transition-colors hover:border-white/20">
                <CardContent className="p-6">
                  <div className="flex items-start gap-4">
                    ...
                    <div className="flex-1">
                      <h3 className="font-semibold text-white">Verify via Email</h3>
                      <p className="mt-1 text-sm text-white/60">
                        We will send a verification code to the email address associated with this profile.
                      </p>
                      <p className="mt-2 text-white/40 text-xs">Requires admin approval after verification</p>
                      <Button className="mt-4" disabled={submitting} onClick={() => initiateClaim("EMAIL_VERIFICATION")} variant="outline">
                        {submitting && claimState.method === "EMAIL_VERIFICATION" ? (
                          <Loader2 className="mr-2 h-4 w-4 animate-spin" />
                        ) : (
            ...
            {!profile.github && (!profile.walletAddresses || profile.walletAddresses.length === 0) && !profile.email && (
              <Card className="border-white/10 bg-white/5 backdrop-blur-md">
                <CardContent className="p-6 text-center">
                  <Shield className="mx-auto mb-4 h-8 w-8 text-white/40" />
                  <p className="text-white/60">
                    This profile does not have any verifiable identifiers (GitHub, wallet address, or email). Please contact support to claim this profile.
                  </p>
                </CardContent>
              </Card>
            )}
            ...
            <div className="rounded-lg border border-white/5 bg-white/[0.02] p-4">
              <div className="flex items-start gap-3">
                ...
                <p className="text-white/40 text-xs leading-relaxed">
                  Claiming a profile links your ecosystem contributions to your Opentribe account. This is a one-time action and cannot be undone. Only claim profiles that belong to you.
                </p>
              </div>
            </div>
          </div>
      ...
      {showAuthModal && (
        <AuthModal isOpen={showAuthModal} onClose={() => setShowAuthModal(false)} redirectTo={`/profile/claim/${slug}`} />
      )}
    </div>
  );
}
