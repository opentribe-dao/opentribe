...
export function useBountiesFilters() {
  ...
  const updateURL = useCallback(
    (newFilters: Partial<BountiesFilters>, replace = true) => {
      const params = new URLSearchParams(searchParams);
      ...
      if (
        ...
        (updatedFilters.priceRange[0] > 0 ||
          updatedFilters.priceRange[1] < 50_000)
      ) {
      ...
      if (
        updatedFilters.hasSubmissions &&
        updatedFilters.hasSubmissions === true
      ) {
  ...
  const updateFilter = useCallback(
    (key: keyof BountiesFilters, value: BountiesFilters[typeof key]) => {
