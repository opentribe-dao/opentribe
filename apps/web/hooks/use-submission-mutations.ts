...
export function useUpdateSubmission(bountyId: string, submissionId: string) {
    const queryClient = useQueryClient();

    return useMutation({
        mutationFn: async (data: SubmissionUpdateData) => {
            const response = await fetch(
                `${API_BASE_URL}/api/v1/bounties/${bountyId}/submissions/${submissionId}`,
                {
                    method: "PATCH",
                    headers: {
        ...
        onSuccess: () => {
            ...
            queryClient.invalidateQueries({
                queryKey: ["bounties", bountyId, "submissions", submissionId],
            });
...
export function useDeleteSubmission(bountyId: string, submissionId: string) {
    const queryClient = useQueryClient();
