...
export const BountyWinnerReminderEmail = ({
  ...
  >
    ...
    <EmailText className="mt-4">
      Your bounty <strong>"{bountyTitle}"</strong> ended {daysPastDeadline} days
      ago. The community is eagerly waiting for the results!
    </EmailText>

    <EmailDivider />

    <EmailCard className="border border-yellow-500/30 bg-yellow-500/10">
      <EmailText className="mb-3 font-semibold text-yellow-400">
        ⚡ Action Required
      </EmailText>
      <EmailText className="text-sm">
        {submissionCount} builders submitted their work and are waiting to hear
        back. Announcing winners promptly helps maintain community trust and
        engagement.
      </EmailText>
    </EmailCard>
    ...
    <EmailCard className="mt-4">
      ...
      <EmailHighlight
        label="Days Since Deadline"
        value={`${daysPastDeadline} days`}
      />
    </EmailCard>

    <EmailButton href={announceUrl}>Select Winners Now</EmailButton>
    ...
    <EmailText className="text-sm text-white/60">
      • Review submission quality and completeness
      <br />• Check if requirements were met
      <br />• Consider community engagement (likes & comments)
      <br />• Distribute prizes according to your tier structure
    </EmailText>
  </BaseTemplate>
);
