...
export const OrgInviteEmail = ({
  ...
  <BaseTemplate
    preview={`You're invited to join ${organizationName} on Opentribe`}
  >
    ...
    <EmailText className="mt-4">
      ...
      on Opentribe as {role === "admin" ? "an Administrator" : "a Member"}.
    </EmailText>

    <EmailDivider />

    <EmailCard>
      <EmailText className="mb-3 font-semibold">Organization Details</EmailText>
      <EmailHighlight label="Organization" value={organizationName} />
      <EmailHighlight
        label="Your Role"
        value={role === "admin" ? "Administrator" : "Member"}
      />
    ...
    {role === "admin" && (
      <EmailCard className="mt-4">
        ...
        <EmailText className="text-sm">
          ...
          <br />• Review submissions
          <br />• Invite new members
          <br />• Update organization settings
        </EmailText>
      </EmailCard>
    )}
    ...
    <EmailText className="text-sm text-white/60">
      This invitation will expire in 7 days. If you don't have an Opentribe
      account yet, you'll be prompted to create one when you accept the
      invitation.
    </EmailText>
  </BaseTemplate>
);
