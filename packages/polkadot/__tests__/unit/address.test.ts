...
describe('Address utilities', () => {
  describe('isValidPolkadotAddress', () => {
    test('should return true for valid Polkadot addresses', () => {
      ...
      expect(isValidPolkadotAddress('15oF4uVJwmo4TdGW7VfQxNLavjCXviqxT9S1MgbjMNHr6Sp5', 0)).toBe(true);
      expect(isValidPolkadotAddress('14E5nqKAp3oAJcmzgZhUD2RcptBeUBScxKHgJKU4HPNcKVf3', 0)).toBe(true);
      expect(isValidPolkadotAddress('1zugcag7cJVBtVRnFxv5Qftn7xKAnR6YJ9x4x3XLgGgmNnS', 0)).toBe(true);
    });

    test('should return true for valid Kusama addresses', () => {
      expect(isValidPolkadotAddress('CpjsLDC1JFyrhm3ftC9Gs4QoyrkHKhZKtK7YqGTRFtTafgp', 2)).toBe(true);
      expect(isValidPolkadotAddress('FHKAe66mnbk8ke8zVWE9hFVFrJN1mprFPVmD5rrevotkcDZ', 2)).toBe(true);
      expect(isValidPolkadotAddress('HNZata7iMYWmk5RvZRTiAsSDhV8366zq2YGb3tLH5Upf74F', 2)).toBe(true);
    });

    test('should return true for valid Westend addresses', () => {
      expect(isValidPolkadotAddress('5GrwvaEF5zXb26Fz9rcQpDWS57CtERHpNehXCPcNoHGKutQY', 42)).toBe(true);
    });
    ...
    test('should return false for invalid addresses', () => {
      ...
      expect(isValidPolkadotAddress('0x742d35Cc6634C0532925a3b844Bc9e7595f0bEb')).toBe(false); // Ethereum address
      expect(isValidPolkadotAddress('1A1zP1eP5QGefi2DMPTfTL5SLmv7DivfNa')).toBe(false); // Bitcoin address
    });

    test('should return false for truncated addresses', () => {
      // Valid address with last character removed
      expect(isValidPolkadotAddress('15oF4uVJwmo4TdGW7VfQxNLavjCXviqxT9S1MgbjMNHr6Sp')).toBe(false);
      expect(isValidPolkadotAddress('5GrwvaEF5zXb26Fz9rcQpDWS57CtERHpNehXCPcNoHGKutQ')).toBe(false);
    });

    test('should return false for addresses with extra characters', () => {
      expect(isValidPolkadotAddress('15oF4uVJwmo4TdGW7VfQxNLavjCXviqxT9S1MgbjMNHr6Sp5!')).toBe(false);
      expect(isValidPolkadotAddress('15oF4uVJwmo4TdGW7VfQxNLavjCXviqxT9S1MgbjMNHr6Sp5X')).toBe(false);
    });

    test('should return false for wrong SS58 format', () => {
      // Polkadot address (SS58 = 0) tested against Kusama format (SS58 = 2)
      expect(isValidPolkadotAddress('15oF4uVJwmo4TdGW7VfQxNLavjCXviqxT9S1MgbjMNHr6Sp5', 2)).toBe(false);

      // Kusama address (SS58 = 2) tested against Polkadot format (SS58 = 0)
      expect(isValidPolkadotAddress('CpjsLDC1JFyrhm3ftC9Gs4QoyrkHKhZKtK7YqGTRFtTafgp', 0)).toBe(false);

      // Westend address (SS58 = 42) tested against Polkadot format
      expect(isValidPolkadotAddress('5GrwvaEF5zXb26Fz9rcQpDWS57CtERHpNehXCPcNoHGKutQY', 0)).toBe(false);
    });
    ...
    test('should use default SS58 format 0 (Polkadot) when not specified', () => {
      ...
      expect(isValidPolkadotAddress('14E5nqKAp3oAJcmzgZhUD2RcptBeUBScxKHgJKU4HPNcKVf3')).toBe(true);
  ...
  describe('isSameAddress', () => {
    ...
    test('should return false for invalid addresses', () => {
      ...
      expect(isSameAddress('15oF4uVJwmo4TdGW7VfQxNLavjCXviqxT9S1MgbjMNHr6Sp5', 'invalid')).toBe(false);
      expect(isSameAddress('invalid', '15oF4uVJwmo4TdGW7VfQxNLavjCXviqxT9S1MgbjMNHr6Sp5')).toBe(false);
    });
