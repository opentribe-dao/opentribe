...
describe("SubscanClient", () => {
  ...
  describe("getBlockDetail", () => {
    test("should fetch block detail successfully", async () => {
      ...
      const result = await client.getBlockDetail({ block_num: 1_000_000 });
      ...
      expect(mockFetch).toHaveBeenCalledWith(
        "https://polkadot.api.subscan.io/api/scan/block",
        {
          ...
          body: JSON.stringify({ block_num: 1_000_000 }),
        }
      );
    });
    ...
    test("should handle HTTP errors", async () => {
      ...
      await expect(
        client.getBlockDetail({ block_num: 1_000_000 })
      ).rejects.toThrow("Subscan HTTP 500");
    });
    ...
    test("should handle API error responses", async () => {
      ...
      await expect(
        client.getBlockDetail({ block_num: 1_000_000 })
      ).rejects.toThrow("Subscan error: Invalid API key");
    });
    ...
    test("should handle missing data in response", async () => {
      ...
      await expect(
        client.getBlockDetail({ block_num: 1_000_000 })
      ).rejects.toThrow("Subscan error: Success");
    });
  });
    ...
    test("should use correct Westend API URL", async () => {
      ...
      mockFetch.mockResolvedValueOnce({
        ...
        json: async () => ({
          code: 0,
          message: "Success",
          data: { block_num: 5_000_000 },
        }),
      });

      await westendClient.getBlockDetail({ block_num: 5_000_000 });
