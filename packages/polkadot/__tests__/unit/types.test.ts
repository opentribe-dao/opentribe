    ...
    test("should reject invalid addresses", () => {
      const invalidAddresses = [
        ...
        { value: "not-an-address", comment: "Invalid characters" },
        {
          ...
          comment: "Extra character",
        },
        {
          value: "15oF4uVJwmo4TdGW7VfQxNLavjCXviqxT9S1MgbjMNHr",
          comment: "Too short (45 chars)",
        },
        {
          value: "1234567890123456789012345678901234567890123456789",
          comment: "Too long (49 chars)",
        },
        {
          value:
      ...
      for (const { value: address, comment } of invalidAddresses) {
        ...
        expect(
          result.success,
          `Expected "${address}" (${comment}) to be invalid`
        ).toBe(false);
        if (!result.success) {
